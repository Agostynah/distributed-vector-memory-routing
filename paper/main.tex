...
Modern vector databases (Milvus, Weaviate, Qdrant) achieve scalability by partitioning embeddings across multiple servers~\cite{distvs,fedvs}. However, querying all partitions---a broadcast---incurs communication costs that grow linearly with the number of partitions $K$, creating a fundamental tension between retrieval quality and system efficiency.
...
HNSW~\cite{hnsw} provides efficient approximate nearest neighbor search through hierarchical graph navigation. Pyramid~\cite{pyramid} proposes pyramid-based partitioning for distributed search. RED-ANNS~\cite{redanns} introduces range-based approaches to distributed approximate nearest neighbor search.
...
\toprule
\textbf{K} & \textbf{Strategy} & \textbf{Recall@10} & \textbf{MRR} & \textbf{NDCG@10} & \textbf{Part. Contacted} & \textbf{Comm Reduction (\%)} & \textbf{Avg Latency (ms)} & \textbf{Avg Confidence} \\
\midrule
\multirow{6}{*}{2}
...
\bibitem{turboquant}
TurboQuant Authors.
\newblock TurboQuant: Data-Oblivious Quantizer with Near-Optimal Distortion.
\newblock In {\em International Conference on Learning Representations (ICLR)}, 2026.

\bibitem{distvs}
Distributed Vector Search Authors.
\newblock Distributed Vector Search: Challenges and Solutions.
\newblock {\em Technical Report}, 2024.

\bibitem{fedvs}
Federated Vector Search Authors.
\newblock FedVS: Towards Communication-Efficient Federated Vector Search.
\newblock {\em Technical Report}, 2024.

\bibitem{dhnsw}
Distributed HNSW Authors.
\newblock Distributed HNSW: Scaling Approximate Nearest Neighbor Search.
\newblock {\em Technical Report}, 2024.

\bibitem{shardmemo}
ShardMemo Authors.
\newblock ShardMemo: Shard-Based Memory with Probe Budget.
\newblock {\em Technical Report}, 2024.

\bibitem{hyphaedb}
HyphaeDB Authors.
\newblock HyphaeDB: Graph-Based Neighbor Routing for Distributed Systems.
\newblock {\em Technical Report}, 2024.

\bibitem{ragroute}
RAGRoute Authors.
\newblock RAGRoute: Routing for Retrieval-Augmented Generation Systems.
\newblock {\em Technical Report}, 2024.

\bibitem{pyramid}
Pyramid Authors.
\newblock Pyramid: Pyramid-Based Partitioning for Distributed Vector Search.
\newblock {\em Technical Report}, 2024.

\bibitem{redanns}
RED-ANNS Authors.
\newblock RED-ANNS: Range-Efficient Distributed Approximate Nearest Neighbor Search.
\newblock {\em Technical Report}, 2024.
